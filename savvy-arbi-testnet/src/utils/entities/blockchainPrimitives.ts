...
export function getOrCreateTransaction(rawTransaction: ethereum.Transaction, rawBlock: ethereum.Block): Transaction {
...
export function createEvent<T extends Entity>(rawEvent: ethereum.Event): T {
    ...
    const transaction = getOrCreateTransaction(rawEvent.transaction, rawEvent.block);

    const event = new Entity();
    event.setString("id", rawEvent.transaction.hash.toHexString() + "/" + rawEvent.logIndex.toString());
    event.setString("block", block.id);
    event.setBigInt("index", rawEvent.logIndex);
    event.setBigInt("timestamp", rawEvent.block.timestamp);
    event.setString("contract", rawEvent.address.toHex());
    event.setString("transaction", transaction.id);

    return changetype<T>(event);
}

export function createContract<T extends Entity>(): T {
    let contract = new Entity();
    
    return changetype<T>(contract);
}