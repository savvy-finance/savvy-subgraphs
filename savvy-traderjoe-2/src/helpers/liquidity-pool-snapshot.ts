...
import { BIGINT_ZERO, BIGDECIMAL_ZERO, QUARTERHOUR_IN_SECONDS } from '../constants';
...
export function getOrCreateLiquidityPoolSnapshot(contractAddress: Address, block: ethereum.Block): LiquidityPoolSnapshot {
  const snapshot = getBeginOfThePeriodTimestamp(block.timestamp, QUARTERHOUR_IN_SECONDS);
  const id = contractAddress.toHexString().concat('-').concat(snapshot.toString());
...
export function createLiquidityPoolSnapshot(contractAddress: Address, block: ethereum.Block, lp: LiquidityPool | null): LiquidityPoolSnapshot {
  const snapshot = getOrCreateLiquidityPoolSnapshot(contractAddress, block);
  if (lp === null) {
