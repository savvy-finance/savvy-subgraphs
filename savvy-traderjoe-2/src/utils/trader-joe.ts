...
export function getLPPairInUSD(
  ...
  const syntheticInPairedAmount = convertSyntheticAmountToPairedAmount(
    binId,
    syntheticAmount,
    pairTokenDecimals
  );
...
export function convertSyntheticAmountToPairedAmount(
  binId: number,
  syntheticAmount: BigInt,
  pairTokenDecimals: number
): BigInt {
  ...
  const priceOfSyntheticInPairedAsset = BigDecimal.fromString(
    `${price * conversionFactor}`
  );
  const normalizedSyntheticInPairedAsset = syntheticAmount
    .toBigDecimal()
    .times(priceOfSyntheticInPairedAsset);
  const syntheticInPairedAsset = normalizedSyntheticInPairedAsset.div(
    BigDecimal.fromString(`${conversionFactor}`)
  );
 ...
 */
export function getBalancesFromBinIds(
  accountAddress: Address,
  lpAddress: Address,
  binIds: BigInt[]
): BigInt[] {
  log.debug("getBalancesFromBinIds -- account: {} lp: {} binIds: {}", [
    accountAddress.toHexString(),
    lpAddress.toHexString(),
    binIds.toString(),
  ]);
  const result = LIQUIDITY_AMOUNTS_CONTRACT.getAmountsOf(
    accountAddress,
    binIds,
    lpAddress
  );
